...
function resolve(id, x) {
	if (/^\./.test(x)) {
		x = decodeURIComponent(url.resolve(id, x));
	}
	return x;
}

function buildSeajsFile(param, srcPath, id, code) {
	if (!code) {
		code = fs.readFileSync(srcPath, 'utf-8');
	}
	// console.log( id );
	var rawDep = collectDep(code);
	rawDep.forEach(function (x) {
		var reg = new RegExp('require[\\s]*\\([\\s]*([\\\'"])' + x.replace(/\//g, '\\/').replace(/\./g, '\\.') + '[\\\'"][\\s]*\\)', 'g');
		code = code.replace(reg, 'require($1' + resolve(id, x) + '$1)');
	});
	...
	rawDep.forEach(function (x, i, a) {
		...
		if (!/^\./.test(z) && !(/^page/.test(z))) {
			z = path.join(param.base, z);
		}
		// console.log( '解析下一个需要读取的文件的路径: \n\t 本文件路径:' + srcPath + ';\n\t\t  require引入的文件相对路径: ' + x  );
		// windows下, 如果是基础文件的全路径, 会影响../查找父级目录, 需要换成srcPath的dirPath
		var depItem = path.resolve(path.dirname(srcPath), z);
		...
		if (G.requriedDep[depItem]) {
			...
			var subID = x;
			if (/^\./.test(x)) {
				subID = decodeURIComponent(url.resolve(id, x));
			}
			// 构建依赖表
...
module.exports = function (param, srcPath, mainID) {
	// console.log( '接收到的参数有: ' );
	...
	buildSeajsFile(param, srcPath, mainID);
